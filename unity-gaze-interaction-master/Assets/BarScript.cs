...
public class BarScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        key_y = PianoKeys[1].transform.position.z;
    }
    ...
    void Update()
    {
        ...
        for (int i = 0; i < 88; i++)
        {
            if (isKeyPressed[i] && barsPressed[i] != null)
            {
                ...
                PianoKeys[i + 1].position = new Vector3(PianoKeyPos.x, PianoKeyPos.y, key_y + 0.015f);
            }
            else if (isKeyPressed[i] == false)
            {
                Vector3 PianoKeyPos = PianoKeys[i + 1].transform.position;
                PianoKeys[i + 1].position = new Vector3(PianoKeyPos.x, PianoKeyPos.y, key_y);
            }
        }
